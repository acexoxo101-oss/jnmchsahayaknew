...
const navItems = [
  ...
  { href: "/doctor/patients", label: "Search Patients", icon: Search },
  { href: "/doctor/reports", label: "Recent Reports", icon: FileText },
  { href: "/doctor/profile", label: "Profile", icon: User },
]

export function DoctorSidebar({ profile }: DoctorSidebarProps) {
      ...
      <div className="p-4 border-b border-sidebar-border">
        <div className="flex items-center gap-3">
          ...
          <div className="flex-1 min-w-0">
            <p className="text-sm font-medium truncate text-sidebar-foreground">Dr. {profile.full_name}</p>
            <p className="text-xs text-sidebar-foreground/70 truncate">
              {profile.department || profile.designation || "Doctor"}
            </p>
          </div>
        </div>
      </div>
